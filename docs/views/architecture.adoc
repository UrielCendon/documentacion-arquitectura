...
Aquí tienes la tabla traducida y actualizada con los nombres exactos que aparecen en tu diagrama de arquitectura:
...
Presentación +
 ...
 └─ Aplicación móvil +
| «capa» Aplicación
| Contiene el «gateway» API Gateway y los servicios de aplicación (App Services) que orquestan los flujos de trabajo y exponen la funcionalidad a la capa de presentación.
Su estructura es la siguiente: +
Aplicación +
├─ «gateway» API Gateway +
├─ «service» AppReservacionServicio +
├─ «service» AppUsuarioServicio +
└─ «service» AppHotelServicio +
| «capa» Dominio
| Contiene el núcleo de la lógica de negocio, descompuesto en subdominios cohesivos identificados mediante DDD. Cada subdominio define sus propias interfaces (contratos) y servicios internos, agrupando la lógica correspondiente. Su estructura es la siguiente: +
Dominio +
├─ Reservaciones +
│ ├─ «interface» IReservacionServicio +
│ └─ ReservacionServicio +
├─ Inventario +
│ ├─ «interface» IInventarioRepositorio +
│ ├─ InventarioRepositorio +
│ ├─ «interface» IInventarioServicio +
│ └─ InventarioServicio +
├─ Pagos +
│ ├─ «interface» IPagosServicio +
│ └─ PagosServicios +
├─ Precios +
│ ├─ «interface» IPoliticaPrecios +
│ └─ PreciosServicio +
├─ Identidad +
│ ├─ «interface» IUsuarioRepositorio +
│ └─ UsuarioServicio +
└─ Catalogos +
      ├─ «interface» ICatalogosServicio +
      └─ CatalogoServicio +
| «capa» Infraestructura
| Contiene las implementaciones técnicas y servicios transversales. Agrupa los repositorios de persistencia (RDBMS), los adaptadores externos y los servicios de soporte.
Su estructura es la siguiente: +
Infraestructura +
├─ «repositorio» SqlReservacionRepositorio +
├─ «repositorio» SqlInventarioRepositorio +
├─ «repositorio» SqlUsuarioRepositorio +
├─ «adaptador» PagosGatewayAdapter +
├─ «transversal» SesionServicio +
└─ «transversal» MonitoreoServicio +
|===
...
- La capa Dominio contiene la lógica de negocio pura, sin dependencias directas de presentación o persistencia. +
- La capa Infraestructura encapsula los detalles técnicos, como las implementaciones SQL (CON-2). +
- La capa Presentación se limita a la interacción con el usuario (CON-6).
...
*Patrón / Decisión:*
El módulo Pagos implementa el patrón Adapter.
Define la interfaz interna `IPagosServicio`, utilizada por el módulo Reservaciones, y una implementación concreta `PagosGatewayAdapter` en la capa de infraestructura que traduce las llamadas hacia la API del proveedor externo certificado.
...
.Diagrama de Comunicación: Consultar Hotel
image::Comunicacion/CU-13-Dar-de-baja-hotel/Consultar-Hotel.png[Consultar Hotel, width=900, align="center"]

.Diagrama de Comunicación: Eliminar Hotel
image::Comunicacion/CU-13-Dar-de-baja-hotel/Eliminar-Hotel.png[Eliminar Hotel, width=900, align="center"]
