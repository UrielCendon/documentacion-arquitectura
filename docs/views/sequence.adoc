...
La vista de proceso representa cómo se orquestan los flujos críticos sobre la arquitectura en capas (Presentación, Aplicación, Dominio e Infraestructura) y el estilo DDD. El modelo de concurrencia es principalmente *orientado a servicios sin estado*, donde cada petición HTTP abre un hilo de ejecución independiente en los App Services. Las operaciones sensibles usan transacciones ACID en la capa de Dominio con consistencia fuerte, las interacciones externas se aíslan con adaptadores que definen *timeouts*, reintentos acotados e idempotencia. Los diagramas de secuencia muestran llamadas sincrónicas (líneas continuas) y callbacks asincrónicos (líneas punteadas) y explican dónde hay paralelismo controlado.

== CU-02 Reservar una habitación

image::Secuencia/CU-02 Reservar una habitación.png[Diagrama de secuencia CU-02, width=900, align="center"]
...
image::Secuencia/CU-03 Realizar el pago de la reservacion.png[Diagrama de secuencia CU-03, width=900, align="center"]
...
Una vez creada la orden, el sistema redirige al cliente a la interfaz de la pasarela, en donde el cliente interactua directamente con el proveedor externo.
...
image::Secuencia/CU-04 Cancelar reservacion.png[Diagrama de secuencia CU-04, width=900, align="center"]
...
La capa de Aplicacion (AppReservacionServicio) concentra la orquestacion, mientras que los servicios de Dominio encapsulan las reglas de negocio (CrearReservacion, CancelarReservacion, procesar pago) y la capa Infrastructure provee repositorios SQL y el adaptador de pagos.
